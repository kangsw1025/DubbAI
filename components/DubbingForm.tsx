...
const STEPS: { key: DubbingStatus; label: string; desc: string }[] = [
  { key: "processing", label: "더빙 처리", desc: "서버에서 더빙 영상을 생성 중..." },
];
const STEP_ORDER: DubbingStatus[] = ["processing", "success"];
...
function ProgressSteps({ status }: { status: DubbingStatus }) {
  return (
    <div className="mt-4 space-y-2">
      {STEPS.map((step) => {
        const state = getStepState(step.key, status, STEP_ORDER);
        return (
...
export function DubbingForm({
  ...
  const activeStatuses: DubbingStatus[] = ["processing"];
  const showProgress =
    isProcessing &&
    isVideoFile &&
    dubbingStatus &&
